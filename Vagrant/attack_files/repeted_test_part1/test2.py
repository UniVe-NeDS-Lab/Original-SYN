    ...
    if ans:
        for _, packet in ans:
            ...
            if tcp_layer.flags == 0x10:  # ACK flag
                logging.info(f"{original_sport} recieved ACK")
...
def infer_backlog_size(target_ip, target_port):
    ...
    while backlog_size <= max_backlog:
        ...
        time.sleep(8)
        #time.sleep(3) #Give some time for packets to be processed
...
#                break
        ...
        time.sleep(80)
        logging.info(f"80 seconds gone")
        time.sleep(10)
...
def wait_for_c():
    while True:
        user_input = input('Press "c" to continue: ').strip().lower()
        if user_input == 'c':
            break
